...
jQuery(function ($) {
    ...
    $('#button_type').on('change', function () {
        if ($(this).val() == 'services') {
            ...
            $('#frequency_count').attr('required', true);
        }
    });
    ...
    $('.angelleye-color-picker').wpColorPicker({
        change: function (event, ui) {
            setTimeout(previewData(), 0);
        },
        clear: function (event, ui) {
            setTimeout(previewData(1), 0);
        },
        hide: true,
        palettes: true
    });
...
function previewData(clear = 0) {
    ...
    if (left_background_color == '' && clear == 1) {
        jQuery('.item-name, .item-price, .shipping-rate .shipping, .tax-amount, .total-amount').css('background-color', '');
    } else {
        // var left_background_color = jQuery('input[name="left_background_color"]').val();
        jQuery('.item-name, .item-price, .shipping-rate .shipping, .tax-amount, .total-amount').css('background-color', left_background_color);
    }

    if (left_foreground_color == '' && clear == 1) {
    ...
    if (right_background_color == '' && clear == 1) {
        jQuery('.item-name, .item-price, .shipping-rate .shipping, .tax-amount, .total-amount').css('background-color', '');
    } else {
        // var right_background_color = jQuery('input[name="right_background_color"]').val();
    ...
    if (right_foreground_color == '' && clear == 1) {
        jQuery('.item-name, .item-price, .shipping-rate .shipping, .tax-amount, .total-amount').css('color', '');
    } else {
        // var right_foreground_color = jQuery('input[name="right_foreground_color"]').val();
